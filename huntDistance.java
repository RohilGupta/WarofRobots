...
/**
 * This is the distance-hunter fighter robot that utilizes GuptaOppData to make calculations
 * @author Rohil G.
 * @date January 24, 2017
 */
public class huntDistance extends FighterRobot{
	...
	/**
	 * The constructor method for the huntDistance fighter robot to be used
	 ...
	 */
	public huntDistance(City c, int a, int s, Direction d,int id, int health){
		super(c,a,s,d,id,health, 4, 2, 4);
		this.health = health;
		this.setLabel();
	}
	...
	public TurnRequest takeTurn(int energy, OppData[] data){
		...
		if (this.targetX == target.getAvenue() && this.targetY == target.getStreet() && energy >= 20){
			this.targetID = target.getID();
			request = new TurnRequest(this.targetX,this.targetY,this.targetID, energy/(100/ATTACK));
		}
	...
	private GuptaOppData getTarget() {
		GuptaOppData[] sortedDistance;
		sortedDistance = sortDistance(); //sorts the distance of each player

		//Checks to see if the lowest distance belongs to this robot
		if (sortedDistance[0].getID() == this.getID()){
	...
	/**
	 * Sorts the distance of each player from lowest to highest
	 * @return - Returns a GuptaOppData object with the sorted health from lowest to highest
	 */
	private GuptaOppData[] sortDistance() {
		GuptaOppData temp;
		...
		for (int index= 1; index<extendedData.length; index++){
			...
			while ((extendedData[previousIndex].getDistance(this.getAvenue(), this.getStreet()) > temp.getDistance(this.getAvenue(), this.getStreet())) && (previousIndex > 0)){
				extendedData[previousIndex + 1] = extendedData[previousIndex]; 
				previousIndex -= 1;
			}

			//Sorts the unsorted array
			if (extendedData[previousIndex].getDistance(this.getAvenue(), this.getStreet()) > temp.getDistance(this.getAvenue(), this.getStreet())){
